...
/**
 * TUTTO CIÒ CHE IL CALCOLO PRENDE DA FUORI.
 *
 * L'elenco non è stato compilato a mano: si è spostato il corpo in un file suo e si è
 * lasciato che il COMPILATORE elencasse i nomi che non trovava più. Sono venticinque —
 * diciannove che il hook riceve dall'editor, sei che si calcola per conto suo.
 */
...
'accompanimentTracks' | 'analysisContextAbsBeat' | 'analysisContexts' | 'analyzedNotes' | 'autoHarmonyLabelOverrides' | 'cadentialPatternsEnabled' | 'currentTonic' | 'enableInferredContexts' | 'harmonyOverrides' | 'inferredContextSuppressions' | 'isAnalysisEnabled' | 'isMinorMode' | 'layoutData' | 'measureLengths' | 'styleProfile' | 'timeSignature' | 'timeSignatureChanges' | 'tonicizationHints' | 'useStatisticalCorrection'> {
    ...
    minSpanBeats: any;
    ornOverrideMap: any;
    ornOverrideRecord: any;
}

/**
 * IL CALCOLO DELLE ETICHETTE, SENZA REACT.
 *
 * Quattromila righe che decidono che cosa è scritto sopra e sotto il rigo: numeri
 * romani, sigle, cifrato, cadenze, tonicizzazioni, contesti dedotti. Stavano dentro un
 * `useMemo`, e finché stavano lì si potevano provare in un modo solo — aprire
 * l'applicazione e guardare. Il corpus di regressione, cinquecento controlli, attraversa
 * il MOTORE e non passa di qui: ogni modifica all'analisi che tocchi le etichette era un
 * salto nel buio.
 *
 * Il corpo è spostato SENZA cambiare una riga di logica: ciò che prima era una variabile
 * catturata dalla chiusura ora è un ingresso dichiarato, e basta.
 *
 * NOTA sugli import: questo file ripete l'intestazione del hook, quindi ne importa anche
 * di non usati. È rumore noto, da ripulire con calma spostando gli aiutanti comuni in un
 * terzo file — non prima, perché toglierli a mano su quattromila righe è il modo
 * migliore per rompere qualcosa senza accorgersene.
 */
export function computeHarmonyLabelsBySystemCore(_in: HarmonyLabelsInput): any[][] {
    const {
        ...
        const autoOverrideByAbsBeat = buildEngineHarmonyOverrideMap(autoHarmonyLabelOverrides || []);
    ...
    for (const [pivBeat, pivInfo] of _pivotCandidates) {
        ...
        timelineFiltered.forEach((event, eventIndex) => {
            ...
            // Fuori dal browser (script, prove) `localStorage` non esiste: il tracer è
            // l'unico punto di questo calcolo che tocchi il mondo esterno, e qui si
            // limita a spegnersi invece di far cadere tutto.
            const _dbgRaw = (typeof localStorage !== 'undefined') ? localStorage.getItem('_HT_DEBUG_BEAT') : null;
