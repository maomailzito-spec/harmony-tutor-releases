/**
 * ESTRAE «che armonia sopra questo basso, sapendo cosa c'e' INTORNO» in
 * `src/data/armoniaNelContestoBasso.json`.
 *
 * Gemella dell'estrattore per la melodia, per il caso che nella didattica viene
 * per primo. Senza di lei la rilettura nel contesto al basso dato non si poteva
 * fare: interrogare la tavola del soprano con una nota di basso porta gli
 * errori da 65 a 81 — misurato, ed e' il motivo per cui era stata esclusa.
 *
...
function main() {
  ...
  for (const nome of files) {
    ...
    for (const ev of (linea || [])) {
      ...
      const basso = suonano.length
        ? suonano.reduce((lo: any, n: any) => (n.midi < lo.midi ? n : lo), suonano[0]) : null;
      G.push(g);
      S.push(basso ? ((((basso.midi % 12) + 12) % 12 - tp + 12) % 12) : -1);
      prec = g;
    }
  ...
  const out = path.resolve(__dirname, '..', 'src', 'data', 'armoniaNelContestoBasso.json');
