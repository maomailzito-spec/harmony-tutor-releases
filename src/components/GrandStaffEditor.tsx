...
const GrandStaffEditor: React.FC<GrandStaffEditorProps> = ({
    ...
    // Live-update playbackDurationTicks on existing acc notes when Ring is toggled.
    // Notes generated by applyAccPattern store playbackDurationTicks only at
    // insertion/revoice time — toggling Ring later needs to patch existing notes.
    useEffect(() => {
        setAccompanimentTracks(prev => prev.map(track => {
            // Compute maxEndTick per chordGroupId
            const groupEndTick = new Map<string, number>();
            for (const n of track.notes) {
                if (n.isRest) continue;
                const gid = (n as any).chordGroupId as string | undefined;
                if (!gid) continue;
                const endTick = Number((n as any).startTick ?? 0) + Number(n.durationTicks ?? 0);
                const cur = groupEndTick.get(gid) ?? 0;
                if (endTick > cur) groupEndTick.set(gid, endTick);
            }
            const updated = track.notes.map(n => {
                if (n.isRest) return n;
                const gid = (n as any).chordGroupId as string | undefined;
                if (!gid) return n;
                const maxEnd = groupEndTick.get(gid);
                if (maxEnd == null) return n;
                const s = Number((n as any).startTick ?? 0);
                const ringDur = maxEnd - s;
                if (accLetRing) {
                    // Only extend if Ring actually makes it longer than the written duration
                    if (ringDur > (n.durationTicks ?? 0)) {
                        return { ...n, playbackDurationTicks: ringDur } as StaffNote;
                    }
                    return n;
                } else {
                    // Remove Ring override if present
                    if (typeof n.playbackDurationTicks === 'number' &&
                        n.playbackDurationTicks > (n.durationTicks ?? 0)) {
                        const { playbackDurationTicks: _dropped, ...rest } = n as any;
                        return rest as StaffNote;
                    }
                    return n;
                }
            });
            return { ...track, notes: updated };
        // Ring is a playback-only config change — must NOT enter the undo history.
        }), { undoable: false });
